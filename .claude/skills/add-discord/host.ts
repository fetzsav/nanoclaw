...
function loadAuth(): DiscordAuth | null {
...
function loadChannelMappings(): ChannelMapping[] {
  const channelsPath = path.join(getDataDir(), 'discord-channels.json');
  if (!fs.existsSync(channelsPath)) {
    return [];
  }
  try {
    const config: ChannelsConfig = JSON.parse(fs.readFileSync(channelsPath, 'utf-8'));
...
function getMappingForChannel(channelId: string): ChannelMapping | undefined {
  return channelMappings.find(m => m.discordChannelId === channelId);
}

function getMappingForGroup(groupFolder: string): ChannelMapping | undefined {
 ...
 */
function shouldTrigger(message: Message, mapping: ChannelMapping): boolean {
 ...
 */
async function sendToChannel(channelId: string, text: string): Promise<boolean> {
  if (!discordClient) {
 ...
 */
export async function initDiscordClient(config: DiscordClientConfig): Promise<void> {
  const auth = loadAuth();
  if (!auth) {
  ...
  discordClient.on('messageCreate', async (message: Message) => {
    ...
    try {
      // Show typing indicator while processing (only for text-based channels that support it)
      if ('sendTyping' in message.channel) {
        await message.channel.sendTyping();
      }

      // Route to container agent
      const response = await config.onMessage(
        message.channelId,
        content,
        senderName,
        mapping.nanoClawGroup,
        mapping.isMain
      );
 ...
 */
export function reloadChannelMappings(): void {
  channelMappings = loadChannelMappings();
